...
총 **6 파일 · 34 테스트** (2026-06 기준)

| 파일 | 대상 모듈 | 테스트 수 | 제품 기능 |
|------|-----------|-----------|-----------|
| `summaryStructured.test.ts` | `summaryStructured.ts` | 8 | Gemini JSON → read/skip·요약 구조 |
| `summaryParse.test.ts` | `summaryParse.ts` | 8 | 표시 제목·레거시 TITLE 파싱 |
| `summaryPrompt.test.ts` | `summaryPrompt.ts` | 6 | Gemini 프롬프트·시스템 지시 |
...
| `detectServiceLang.test.ts` | `detectServiceLang.ts` | 2 | 브라우저 언어 → UI 언어 |
...
| 테스트 | 검증 내용 |
|--------|-----------|
| valid JSON | `readRecommendation`, `readReason`, `title`, `briefLines`(3), `fullSummary` 정상 매핑 |
...
**함수:** `langTag`, `buildLeanPrompt`, `buildSummaryFormatGuide`, `buildSystemInstructionForLang`

| 테스트 | 검증 내용 |
|--------|-----------|
| `langTag` | `Chinese` → `zh`, 그 외 → `en` |
| English prompt | `LANG=en`, 제목·본문 포함, `readRecommendation`·`briefLines` 스키마 안내 |
| Chinese prompt | `LANG=zh`, `OUTPUT_FIXED=zh-CN` |
| format guide | English 가이드에 `exactly 3 strings` |
| system instruction | JSON 필드명·`简体中文` / English 지시 포함 |
...
| `geminiClient.ts` | 실 API·`fetch` mock 필요 (추후 확장 가능) |
